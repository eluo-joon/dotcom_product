		   ...
		   if ('A' == plink_type) { // URL 링크 
			   ...
			   wDicProd.lnkPhonePopBtn(plink_url); 
		        ...
		        $j.ajax({
		    		...
		    		data: { "socCode": psoc_code, "itemCode": pitemCode },
		   ...
		   if ('A' == $j('#selLinkType').val()) { // URL 링크 
			   ...
			   wDicProd.lnkPhonePopBtn($j('#selLinkUrl').val()); 
		        ...
		        $j.ajax({
					dataType: "html",
					url: '/wDic/reqInfoPopup.ajax',
					data: { "socCode": $j('#selSocCode').val(), "itemCode": $j('#selItemCode').val() },
					success: function(data) {
						$j("#reqInfoPopup").html(data);
					}
				});				
...
var compareBox = {
    setScroll: function() {
        ...
        if (viewScrollTop > fareLlistTop && viewScrollTop < fareLlistTop + fareLlistHeight - offSet) {
            floatingClass.stop().animate({ top: viewScrollTop - fareLlistTop + offSet }, 500, 'easeOutExpo');
        } else {
            floatingClass.stop().animate({ top: 0 }, 500);
        }
    },
...
var mainScript = {
    ...
    getSubscriptionProductInfo: function(idx) {
        $j.ajax({
            ...
            data: { "idx": idx, "subScpnCode": subScpnCode },
            success: function(data) {
                $j(".bottom-area").empty();
		...
		return;
			...
			$j.ajax({
				...
				,success:function(rv) {
					...
					if(kind_use == "LTE"){
					$j("html,body").animate({scrollTop:800});
					}else{
					$j("html,body").animate({scrollTop:700});
					}
				}
			});
