            ...
            var l = function() {
                ...
                g = function() {
                    if (s.settings.infiniteLoop && "fade" !== s.settings.mode && !s.settings.ticker) {
                        ...
                        s.settings.ariaHidden && (i.attr("aria-hidden", !0).find('a, iframe, button').attr('tabindex', '-1'),
                                n.attr("aria-hidden", !0).find('a, iframe, button').attr('tabindex', '-1')),
                ...
                b = function() {
                    for (var e = "", i = "", n = f(), o = 0; o < n; o++)
                        ...
                        e += '<div class="bx-pager-item"><a href="" data-slide-index="' + o + '" class="bx-pager-link" role="button">' + i + "<span>번째 항목</span></a></div>";
                    s.pagerEl.html(e)
                },
                w = function() {
                ...
                C = function() {
                    s.controls.next = t('<a class="bx-next" href="" role="button">' + s.settings.nextText + "</a>"),
                        s.controls.prev = t('<a class="bx-prev" href="" role="button">' + s.settings.prevText + "</a>"),
                        s.controls.next.bind("click touchend", E),
                        s.controls.prev.bind("click touchend", k),
                        s.controls.prev.bind("focusin", y),
                ...
                T = function() {
                    s.controls.start = t('<div class="bx-controls-auto-item"><a class="bx-start" href="" role="button">' + s.settings.startText + "</a></div>"),
                        s.controls.stop = t('<div class="bx-controls-auto-item"><a class="bx-stop" href="" role="button">' + s.settings.stopText + "</a></div>"),
                ...
                M = function(t) {
                    o.startAuto(), s.controls.autoEl.find('.bx-stop').focus(),
                        t.preventDefault()
                },
                y = function(t) {
                    o.stopAuto(),
                    ...
                    if (t.target.className === 'bx-stop') {
                        s.controls.autoEl.find('.bx-start').focus();
                    }
                },
                z = function(e) {
                    var i, n;
                ...
                I = function(e) {
                    ...
                    return "short" === s.settings.pagerType ? (s.settings.maxSlides > 1 && (i = Math.ceil(s.children.length / s.settings.maxSlides)),
                        void s.pagerEl.html(e + 1 + s.settings.pagerShortSeparator + i)) : (s.pagerEl.find("a").removeClass("active").removeAttr('title'),
                        void s.pagerEl.each(function(i, n) {
                            t(n).find("a").eq(e).addClass("active").attr('title', '선택됨')
                        }))
                },
                q = function() {
                    if (s.settings.infiniteLoop) {
                ...
                A = function(t) {
                    s.settings.autoControlsCombine ? s.controls.autoEl.html(s.controls[t]) : (s.controls.autoEl.find("a").removeClass("active"),
                        s.controls.autoEl.find(".bx-" + t).addClass("active"))
                },
                D = function() {
                ...
                R = function(t) {
                    ...
                    s.touch.end.x = i[0].pageX,
                        ...
                        "fade" === s.settings.mode ? (r = Math.abs(s.touch.start.x - s.touch.end.x),
                            ...
                            n = s.touch.originalPos.top), !s.settings.infiniteLoop && (0 === s.active.index && r > 0 || s.active.last && r < 0) ? S(n, "reset", 200) : Math.abs(r) >= s.settings.swipeThreshold ? (r < 0 ? o.goToNextSlide() : o.goToPrevSlide(),
                ...
                B = function(t) {
                    ...
                    s.settings.ariaHidden && !s.settings.ticker && (s.children.attr("aria-hidden", "true").find('a, iframe, button').attr('tabindex', '-1'),
                        s.children.slice(t, t + e).attr("aria-hidden", "false").find('a, iframe, button').attr('tabindex', '0'));
                    s.children.slice(t, t + e).find('a, button').on('focusin', function() {
                        o.stopAuto();
                    });

                    if (s.children.length < (t + e)) {
                        var tsum = e - (s.children.length - t);

                        o.children('.bx-clone').attr("aria-hidden", "true").find('a, iframe, button').attr('tabindex', '-1');
                        for (var i = 1; i <= tsum; i++) {
                            o.children().eq(((s.children.length - 1) + e) + i).attr("aria-hidden", "false").find('a, iframe, button').attr('tabindex', '0');
                        }
                    } else {
                        o.children('.bx-clone').attr("aria-hidden", "true").find('a, iframe, button').attr('tabindex', '-1');
                    }
